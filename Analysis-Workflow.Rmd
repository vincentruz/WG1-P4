...
#Chem
m1.a_chem <- glm(aptaker ~ factor(firstgen) + factor(lowincomeflag)  + factor(female) + factor(urm) +
                 scale(hsgpa) + scale(mathsr) + scale(englsr) + factor(cohort),
...
#Physics
m1.a_phys <- glm(aptaker ~ factor(firstgen) + factor(lowincomeflag)  + factor(female) + factor(urm) +
                 scale(hsgpa) + scale(mathsr) + scale(englsr) + factor(cohort),
...
#Chem
m1.b_chem <- lm(scale(apscore) ~ factor(firstgen) + factor(lowincomeflag)  + factor(female) + factor(urm) +
                scale(hsgpa) + scale(mathsr) + scale(englsr) + factor(crs_term), 
              df_CHtakers)
summary(m1.b_chem)   

#Physics
m1.b_phys <- lm(scale(apscore) ~ factor(firstgen) + factor(lowincomeflag)  + factor(female) + factor(urm) +
                scale(hsgpa) + scale(mathsr) + scale(englsr) + factor(crs_term), 
              df_PHtakers)
summary(m1.b_phys)   
...
```{r echo = FALSE}
print("Example: Model Parameters for RQ1b - Bio")
...
```{r echo = FALSE}
print("Example Data Viz Dataframe")
as_tibble(df_viz)
```
...
plot1 <- ggplot(data=df_viz, aes(y = numgrade_2, x = apscore_full, color = as.factor(skipped_course), 
                                     fill = as.factor(skipped_course), na.omit = TRUE)) +
      geom_point(stat = 'summary', fun.y = 'mean', size=3) +
      geom_errorbar(aes(ymin = numgrade_2.mean - numgrade_2.se, 
                        ymax = numgrade_2.mean + numgrade_2.se), width=0.1) + #add standard error bars
...
``` {r include = FALSE}
plot2 <- ggplot(data=df_viz, aes(y = numgrade_2.fitted, x = apscore_full, color = as.factor(skipped_course), fill = as.factor(skipped_course), na.omit = TRUE)) +
      geom_point(stat = 'summary', fun.y = 'mean', size=3) +
      ...
      labs(x = "AP Score", y = paste("Mean Grade in", "CHEM", "2"), title= paste("CHEM", "Controlled Model"))
...
``` {r include = FALSE}
plot3 <- ggplot(data=df_viz %>% filter(apscore_full != 0), aes(x = apscore_full, color = as.factor(skipped_course), fill = as.factor(skipped_course), na.omit = TRUE)) +
