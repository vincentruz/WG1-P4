...
### a.	For each subject, generate column of fitted values; then combine disciplines into single dataframe for viz
``` {r include = FALSE}
# BIO
# Generate estimates
...
## 1. Build interactive viz using Shiny
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(inputId = "discipline", 
                  ...
                  selected = unique(df_viz$discipline)[1]),
...
server <- function(input, output) {
  ...
  chartData <- reactive({
    df_viz %>%
      ...
      mutate(numgrade_2.mean = mean(numgrade_2, na.rm = TRUE),
             ...
             fitted.mean = mean(fitted, na.rm = TRUE),
             fitted.sd = sd(fitted, na.rm = TRUE),
             fitted.n = sum(!is.na(fitted)),
             fitted.se = fitted.sd / sqrt(fitted.n))
  })
  ...
  output$linePlotUncontrolled <- renderPlot({
    ggplot(data=chartData(), aes(y = numgrade_2, x = apscore_full, color = skipped_course, fill = skipped_course,
                                 na.omit = TRUE)) +
  ...
  output$linePlotFitted <- renderPlot({
    ggplot(data=chartData(), aes(y = fitted, x = apscore_full, color = skipped_course, fill = skipped_course, na.omit = TRUE)) +
      geom_point(stat = 'summary', fun.y = 'mean', size=3) +
      geom_errorbar(aes(ymin = fitted.mean - fitted.se, ymax = fitted.mean + fitted.se), width=0.1) +
